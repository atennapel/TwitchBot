	...
	if(this.enableWhispers) {
		...
		this.whisperbot.addListener('raw', (function(msg) {
			if(msg.command === 'WHISPER' && msg.args[0] === this.name) {
				...
				this.tryCommands(from, this.name, message, msg);
				for(var i = 0, a = this._whispercb, l = this._whispercb.length; i < l; i++) a[i](from, message);
			}
		}).bind(this));
...
TwitchBot.prototype.onWhisper = function(f) {this._whispercb.push(f); return this};
