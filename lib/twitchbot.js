...
TwitchBot.prototype.run = function() {
	...
	if(this.enableWhispers) {
		...
		/*
		this.whisperbot.addListener('message', console.log.bind(console, '@WHISPER-message'));
		this.whisperbot.addListener('names' + this.channel, console.log.bind(console, '@WHISPER-names'));
		this.whisperbot.addListener('join' + this.channel, (function(nick, message) {
			console.log('@WHISPER-join', nick, message);
			if(nick === this.name) {
				console.log('@WHISPER-JOINED');
				// this.whisperbot.send('CAP', 'REQ', 'twitch.tv/commands');
				this.whisper('labbekak', 'hi!');
			}
		}).bind(this));
		this.whisperbot.addListener('part' + this.channel, console.log.bind(console, '@WHISPER-part'));
		this.whisperbot.addListener('+mode', console.log.bind(console, '@WHISPER+mode'));
		this.whisperbot.addListener('-mode', console.log.bind(console, '@WHISPER-mode'));*/
